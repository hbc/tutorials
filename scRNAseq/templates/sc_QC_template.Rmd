...
raw_counts <- raw_counts[which(rownames(raw_counts) %in% gene_symbols$Row.names), ]

gene_symbols <- gene_symbols[which(gene_symbols$Row.names %in% rownames(raw_counts)), ]

# all(gene_symbols$Row.names == rownames(raw_counts))
...
# Add sample name
sample1 <- which(str_detect(metadata$cells, "run1_ATAGAGAG"))
sample2 <- which(str_detect(metadata$cells, "run1_ATTAGACG"))
sample3 <- which(str_detect(metadata$cells, "run1_CTCCTTAC"))
sample4 <- which(str_detect(metadata$cells, "run1_TACTCCTT"))

metadata$sample <- "x"
metadata$sample[sample1] <- "Good name for sample1"
metadata$sample[sample2] <- "Good name for sample2"
metadata$sample[sample3] <- "Good name for sample3"
metadata$sample[sample4] <- "Good name for sample4"
#which(metadata$sample == "x")

# Get the total number of reads per sample
nReads_sample1 <- read_tsv("path/to/final/run1-ATAGAGAG/run1-ATAGAGAG-barcodes.tsv", col_names = c("cell", "nReads"))  %>% 
        mutate(cell = make.names(cell)) %>%
        mutate(sample = "Good name for sample1")

nReads_sample2 <- read_tsv("path/to/final/run1-ATTAGACG/run1-ATTAGACG-barcodes.tsv", col_names = c("cell", "nReads"))  %>% 
        mutate(cell = make.names(cell)) %>%
        mutate(sample = "Good name for sample2")

nReads_sample3 <- read_tsv("path/to/final/run1-CTCCTTAC/run1-CTCCTTAC-barcodes.tsv", col_names = c("cell", "nReads"))  %>% 
        mutate(cell = make.names(cell)) %>%
        mutate(sample = "Good name for sample3")

nReads_sample1 <- read_tsv("path/to/final/run1-TACTCCTT/run1-TACTCCTT-barcodes.tsv", col_names = c("cell", "nReads"))  %>% 
        mutate(cell = make.names(cell)) %>%
        mutate(sample = "Good name for sample4")

nReads <- rbind(nReads_sample1, nReads_sample2, nReads_sample3, nReads_sample4)
